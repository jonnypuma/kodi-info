...
- **Multiple Kodi servers**: Each defined **`KODI_HOST`** and **`KODI_HOST_1` … `_10`** is its **own** dropdown row—**no merging** (**`KODI_LABEL`** / **`KODI_LABEL_N`** for names); manual host/port on the starter screen — Scan/Clean use the connection you loaded last (signed session cookie); **recent custom servers**: focusing the custom host field shows up to **10** previously used combos stored in **`localStorage`** (host, port, scheme, username—password never stored)
...
# Per-slot KODI_USERNAME_N / KODI_PASSWORD_N override the globals below; if omitted,
# slots inherit KODI_USERNAME / KODI_PASSWORD automatically.
