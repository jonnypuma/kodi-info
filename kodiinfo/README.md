...
- Kodi HTTP JSON-RPC confirms that a Scan/Clean request was accepted, but does not reliably report scanner completion. The UI therefore shows an `accepted` state and elapsed duration, then performs an automatic dashboard reload without claiming that Kodi has finished.
