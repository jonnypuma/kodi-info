...
class KodiLibraryProbe:
    ...
    def __init__(self, host: str, port: int = None, username: str = "", password: str = ""):
        ...
        Args:
            host: Kodi device URL (e.g., http://192.168.0.120:6666) or IP address
...
<head>
    ...
    <link rel="icon" href="/favicon.ico" sizes="32x32" type="image/x-icon">
        ...
        .stat-column h2 {{ margin: 0 0 10px 0; color: white; text-align: center; font-size: 1.5em; }}
        ...
        .recent-item h3 {{ margin-top: 0; color: white; }}
...
# Debug logging removed for security
    ...
    print(f"📊 Access statistics at: http://localhost:{web_port}")
    print(f"🔗 Use this URL in Homarr iframe: http://localhost:{web_port}")
...
Examples:
  ...
  python kodi_info.py --host http://192.168.0.120:6666 --save-html
  python kodi_info.py --host http://192.168.0.120:6666 --web-server --web-port 5005
        """
    )
    
    parser.add_argument('--host', help='Kodi device URL (e.g., http://192.168.0.120:6666) or IP address')
    ...
    if args.web_server:
        create_web_server(args.web_port)
        return
